...
base_filter = [
    ...
    {"term": {"data_stream.dataset": "panw.panos"}},
...
if SAMPLING_METHOD == "uniform":
    ...
    per_bucket = math.ceil(MAX_PANOS_LOGS / n_buckets)
    ...
    quotas = {b["key"]: max(1, round(MAX_PANOS_LOGS * b["count"] / total)) for b in minute_buckets}
...
if SAMPLING_METHOD == "uniform":
    ...
    if allocated > MAX_PANOS_LOGS:

        last_key = minute_buckets[-1]["key"]

        quotas[last_key] -= allocated - MAX_PANOS_LOGS
...
for bucket in minute_buckets:
    ...
    if total_in_bucket <= desired:
        ...
        q = {
            ...
            "query": {
                ...
                "function_score": {
                    ...
                    "random_score": {
                        ...
                        "field": "_seq_no"          # stable across runs
            ...
            "sort": [{"@timestamp": "asc"}]   # final chronological order
...
final_sample = all_hits[:MAX_PANOS_LOGS]

 

print(f"\nFinal sample size: {len(final_sample)} (target {MAX_PANOS_LOGS})")

 

with open("panos_rest_golden.json", "w") as f:

    json.dump(final_sample, f, indent=2)

 

print("Saved panos_rest_golden.json")