...
base_filter = [
    ...
    {"term": {"data_stream.dataset": "endpoint.events.network"}},
...
if SAMPLING_METHOD == "uniform":
    ...
    per_bucket = math.ceil(MAX_LOGS / n_buckets)
    ...
    quotas = {b["key"]: max(1, round(MAX_LOGS * b["count"] / total)) for b in minute_buckets}
...
if SAMPLING_METHOD == "uniform":
    ...
    if allocated > MAX_LOGS:

        last_key = minute_buckets[-1]["key"]

        quotas[last_key] -= allocated - MAX_LOGS
...
final_sample = all_hits[:MAX_LOGS]

 

print(f"\nFinal sample size: {len(final_sample)} (target {MAX_LOGS})")

 

with open("endpoint_events_network_golden.json", "w") as f:

    json.dump(final_sample, f, indent=2)

 

print("Saved endpoint_events_network_golden.json")